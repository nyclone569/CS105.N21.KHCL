...
function init() {
    ...
    let floorMat = new THREE.MeshPhongMaterial({ color: 0x787878, side: THREE.DoubleSide })
...
function initGUI() {
    ...
    h.add(settings.common, 'background', ['texture', 'space texture', 'color']).onChange(() => {
        if (settings.common.background === 'texture') {
            var backgroundTexture = cubeTextureLoader.setPath('img/textures/background/').load(['px.jpg', 'nx.jpg', 'py.jpg', 'ny.jpg', 'pz.jpg', 'nz.jpg']);
            scene.remove(floorMesh)
        }
        else if (settings.common.background === 'space texture') {
            var backgroundTexture = textureLoader.load("img/textures/background/space_2.jpg")
            scene.remove(floorMesh)
        }
        else if (settings.common.background === 'color') {
            var backgroundTexture = new THREE.Color(settings.common.color)
            scene.add(floorMesh)
        }
    ...
    g.add(settings.geometry, 'material', ['points', 'basic', 'lambert', 'phong', 'lines', 'wireframe', 'wood texture', 'earth texture', 'sun texture', 'moon texture']).onChange(() => {
        if (settings.geometry.material === 'basic') {
            ...
            scene.remove(mesh)
            material = new THREE.PointsMaterial({ color: settings.geometry.color, size: 0.1 })
        ...
        } else if (settings.geometry.material === 'wood texture') {
            scene.remove(mesh)
            ...
            mesh = new THREE.Mesh(geometry, material)
        } else if (settings.geometry.material === 'sun texture') {
            scene.remove(mesh)
            var sun = textureLoader.load('img/textures/planets/sun.jpg')
            material = new THREE.MeshBasicMaterial({ map: sun, side: THREE.DoubleSide })
            mesh = new THREE.Mesh(geometry, material)
        } else if (settings.geometry.material === 'moon texture') {
            scene.remove(mesh)
            var moon = textureLoader.load('img/textures/planets/moon.jpg')
            material = new THREE.MeshBasicMaterial({ map: moon, side: THREE.DoubleSide })
            mesh = new THREE.Mesh(geometry, material)
        }
        mesh.position.set(0, 0.5, 0)
        mesh.castShadow = true
    ...
    g.addColor(settings.geometry, 'color').onChange(() => {
        if (settings.geometry.material === 'points') {
            material = new THREE.PointsMaterial({ color: settings.geometry.color, side: 0.1 })
